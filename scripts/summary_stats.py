...
def one_sample(df, inc_min, inc_max, qmax):
    sub = apply_cuts(df, inc_min, inc_max, qmax)
    lam = sub["lambda"].astype(float).to_numpy()
    W, p = shapiro(np.log10(lam))
...
def main():
    ...
    S = cfg["samples"]
    strict   = one_sample(df, S["strict"]["inc_min"],   S["strict"]["inc_max"],   S["strict"]["Q_max"])
    extended = one_sample(df, S["extended"]["inc_min"], S["extended"]["inc_max"], S["extended"]["Q_max"])
